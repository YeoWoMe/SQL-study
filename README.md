### 📆 대면 스터디 일정

| 날짜                 | 주요 내용                                                         |
|----------------------|------------------------------------------------------------------|
| 4/11(토) 13시           | 환경 구축 점검, SQL 입문 문법 복습 및 문제 풀이                  |
| 미정 (5월 1~2주차)   | SQL 핵심 문법 복습 및 문제 풀이, 해설                            |
| 미정 (5월 3~4주차)   | 실전 분석용 SQL 문법 복습 및 데이터 분석 프로젝트 논의           |

대면 스터디에서는 개념 리뷰 후 문제(2문제 예상) 풀이 후 함께 해설하는 방식으로 진행할 예정

### 🏫 활용 강의
[Do it! MySQL로 배우는 SQL 입문](https://www.youtube.com/playlist?list=PLG7te9eYUi7uFBOZYE1-24UYq53u1qmxW)

* 각 주차별 강의 수강 후 공부 기록 폴더에 게시 <br>
* 코드 파일, 필기 정리 등 자신이 공부한 기록이면 다 👌

| 주차 | 기간        | 강좌        | 주요 내용                         |
|------|------------|------------|----------------------------------|
| 1주차 | 4/2 - 4/4  | 1, 2강     | DB 입문 및 MySQL 환경 구축        |
| 2주차 | 4/5 - 4/11 | 3강        | SQL 입문 문법                     |
| 3주차 | 4/26 - 5/2 | 4강        | SQL 기본 문법                     |
| 4주차 | 5/3 - 5/9  | 5강        | JOIN 문법                         |
| 5주차 | 5/10 - 5/16| 6강        | 다양한 SQL 함수                   |
| 6주차 | 5/17 - 5/23| 7, 8강     | 활용 및 크롤링 후 데이터 처리     |

### 👨‍💻👩‍💻 SQL 문제 풀이 
주차별로 3-5문제씩 풀이 후 대면 스터디에서 함께 해설 <br>
<문제 사이트> <br>
[HackerRank](https://www.hackerrank.com/domains/sql) <br>
[Leetcode](https://leetcode.com/studyplan/top-sql-50/) <br>
[Programmers](https://school.programmers.co.kr/learn/challenges?tab=sql_practice_kit)
| 주차 | 기간        | 문제        |
|------|------------|------------|
| 2주차 | 4/5 - 4/11 |HackerRank revising the select query, select by ID, Weather observation station1 | 
| 3주차 | 4/26 - 5/2 |         |
| 4주차 | 5/3 - 5/9  |HackerRank Top Competitors, weather observation station 8, Average Population of Each Continent |
| 5주차 | 5/10 - 5/16|https://school.programmers.co.kr/learn/courses/30/lessons/131117 <br> https://school.programmers.co.kr/learn/courses/30/lessons/59045 <br> https://school.programmers.co.kr/learn/courses/30/lessons/157340          |
| 6주차 | 5/17 - 5/23|         |
